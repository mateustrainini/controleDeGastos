        ...
        row.innerHTML = `
            ...
            <td>
                <div class="actions-container">
                    <button type="button" class="action-button btnEditExpense" data-id="${expense.id}"><span class="bi bi-pencil" style="color: #ffd000;"></span></button>
                    <button type="button" class="action-button btnDeleteExpense" data-id="${expense.id}"><span class="bi bi-trash" style="color: red;"></span></button>
                </div>
            </td>
        `;
        expenseTableBody.appendChild(row);

        let btnEditExpense = row.querySelector('.btnEditExpense');
        btnEditExpense.addEventListener('click', () => { openEditExpenseModal(index) });

        let btnDeleteExpense = row.querySelector('.btnDeleteExpense');
        btnDeleteExpense.addEventListener('click', () => { deleteExpense(index) });
    });
}
